...
public class ResizeImage {
    static Logger logger = LogManager.getLogger(ResizeImage.class);
    public static boolean resizeImage(String imagePath, int width, int height, String extension, String fileName, String destiny, boolean overwriteDestinationFile) throws IOException {
        ...
        try {
            File file = new File(destiny + nomeImagem);
            if(!file.isFile() || overwriteDestinationFile)
            {
            ...
            logger.debug("Image file "+nomeImagem+" already exists at destination path "+destiny+" ; overwrite option is :" + overwriteDestinationFile);
